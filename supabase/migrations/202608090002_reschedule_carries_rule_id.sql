-- reschedule_todo's replacement-row insert omitted schedule_rule_id, so
-- rescheduling one occurrence of a task-mode recurring rule silently detached
-- it from the series -- completing the rescheduled row would never
-- materialize the next occurrence, since todos PATCH's completion trigger
-- reads schedule_rule_id off the row it just updated.
...
begin
  ...
  insert into public.todos (
    ...
    original.source, original.is_recurrence_exception, original.schedule_rule_id,
