-- R1-3 (Reminder v2): reschedule_todo only copied the legacy scalar
-- reminder_offset_minutes onto the replacement row, so rescheduling an
-- occurrence with multiple reminders (reminder_offsets_minutes) silently
-- dropped every reminder past the first. Widen the insert to also carry the
-- array through -- original.reminder_offsets_minutes is already available
-- via `select *` into the existing original%rowtype var, no new select
-- needed.
...
begin
  ...
  insert into public.todos (
    ...
    reminder_offset_minutes, reminder_offsets_minutes, sort_order, status,
    progress, source, is_recurrence_exception, schedule_rule_id,
    rescheduled_from_id, version, creation_request_hash
  ) values (
    ...
    original.reminder_offset_minutes, original.reminder_offsets_minutes,
    original.sort_order, 'planned', 0,
