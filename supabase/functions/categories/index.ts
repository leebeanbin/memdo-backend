  ...
  fetch: withApi<any>(async (request, context, currentRequestId) => {
    ...
    if (request.method === 'GET') {
      ...
      if (error) {
        ...
        return apiError('INTERNAL_ERROR', '카테고리를 불러오지 못했습니다.', 500, currentRequestId)
      }
    ...
    if (request.method === 'PUT') {
      const parsed = categoriesReplaceSchema.safeParse(await request.json().catch(() => undefined))
      if (!parsed.success) {
        return apiError(
          'INVALID_REQUEST',
          '카테고리 목록을 확인해 주세요.',
      ...
      try {
        ...
        return apiError('INTERNAL_ERROR', '카테고리를 저장하지 못했습니다.', 500, currentRequestId)
      }
